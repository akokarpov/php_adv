...
class Catalog extends BasePage {
    ...
    public function action_edit() {
        $catalog = new CatalogModel;
        if($_POST) {
            $catalog->editGood($_POST);
        }
        $message = ($_GET['message'] == 'edited') ? "<h2 style='color: darkgreen'>Товар успешно сохранен!</h2><br>" : null;
        $goods = $catalog->getAllGoods();
        $this->title = 'Список товаров';
        $this->content = $this->templater('views/goodEdit.php', array('message' => $message, 'goods' => $goods));
    }
