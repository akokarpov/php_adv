...
class CatalogModel {
    ...
    function getAllGoods() {
        ...
        $limit = ($_SESSION['admin'] == true) ? "" : "limit {$_SESSION['goodsLimit']}";
        $query = "select * from goods order by id desc $limit";
    ...
    function editGood($post) {
        $DB = DB::getDB();
        $connect = $DB->getConnect();    
        $query = "UPDATE goods
        SET title='{$post['title']}', image='{$post['photo']}', spec='{$post['spec']}', price='{$post['price']}'
        WHERE id = {$post['id']}";
        if(mysqli_query($connect, $query)) {
            header('Location: /?controller=catalog&action=edit&message=edited');
        }
    }
